...
from app.infrastructure.db.models import Board, BoardColumn
...
class BoardColumnRepository:
    ...
    async def get_by_board_id(db: AsyncSession, board_id: int) -> list[BoardColumn]:
        stmt = (
            select(BoardColumn)
            .where(BoardColumn.board_id == board_id, BoardColumn.deleted_at.is_(None))
    ...
    async def get_by_public_id(db: AsyncSession, public_id: UUID) -> BoardColumn | None:
        stmt = (
            select(BoardColumn)
            .options(selectinload(BoardColumn.board).selectinload(Board.members))
            .where(BoardColumn.public_id == public_id, BoardColumn.deleted_at.is_(None))
        )
    ...
    async def get_by_id(db: AsyncSession, column_id: int) -> BoardColumn | None:
        ...
        stmt = select(BoardColumn).where(BoardColumn.id == column_id, BoardColumn.deleted_at.is_(None))
