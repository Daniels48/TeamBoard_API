...
from app.modules.columns.schema import BoardColumnCreate, BoardColumnResponse, BoardColumnUpdate
from app.modules.columns.service import BoardColumnService
...
async def update_column(board_id: UUID, column_id: UUID, data: BoardColumnUpdate, db: DBSession, user: CurrentUser):
    return await BoardColumnService.update_column(
        db=db, board_public_id=board_id, column_public_id=column_id, user=user, data=data
    )
