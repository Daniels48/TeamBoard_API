...
from app.infrastructure.db.models import Board, BoardColumn, BoardMember, Card, User
...
class BoardRepository:
    ...
    async def get_by_public_id(db: AsyncSession, public_id: UUID) -> Board | None:
        stmt = (
            ...
            .where(Board.public_id == public_id, Board.deleted_at.is_(None))
        )
    ...
    async def get_board_members(db: AsyncSession, public_id: UUID) -> Board:
        stmt = (
            select(Board)
            .options(selectinload(Board.owner), selectinload(Board.members).selectinload(BoardMember.user))
            .where(Board.public_id == public_id)
        )
        result = await db.execute(stmt)
