...
from app.modules.members.schema import BoardMemberResponse, AddBoardMemberRequest, BoardMembersResponse, UpdateMemberRoleRequest
...
async def update_member_role(board_id: UUID, username: str,data: UpdateMemberRoleRequest, db: DBSession, current_user: CurrentUser):
   return await BoardMemberService.update_role(db=db,board_public_id=board_id, username=username, role=data.role, current_user=current_user)