...
from app.infrastructure.db.models import BoardMember, User
...
class BoardMemberService:
    ...
    async def add_member(
        db: AsyncSession, board_public_id: UUID, current_user: User, data: AddBoardMemberRequest
    ) -> BoardMemberResponse:
    ...
    async def update_role(
        db: AsyncSession, board_public_id: UUID, username: str, role: BoardRole, current_user: User
    ) -> BoardMemberResponse:
        UserPolicy.require_verified_email(user=current_user)
        board = await BoardRepository.get_by_public_id(db=db, public_id=board_public_id)
        ...
        target_user = await UserRepository.get_by_username(
            db=db,
            username=username,
        )
